...
**Migrations run automatically on deploy.** `npm run vercel-build` (which Vercel prefers over `build`) runs `drizzle-kit migrate` before `next build`, so a deploy that adds a table can never ship code that queries a table the database doesn't have — that mismatch takes the site down. Migrations are idempotent (tracked in `drizzle.__drizzle_migrations`), and a failed migration fails the build rather than deploying broken code. Two caveats: preview deployments run migrations too, so while previews share the production `DATABASE_URL` a preview branch will migrate the live database — give previews their own Neon branch before that matters; and a destructive migration still needs the usual care, because the build applies it without asking.
